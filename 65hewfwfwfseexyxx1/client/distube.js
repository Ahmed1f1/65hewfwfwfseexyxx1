...
distube.on(`playSong`, (queue, song) => {
  try {
    queue.textChannel.send({
      content: `> <a:VorRteX:1171926655078506547>  *Playing*: **${song.name}** - \`(${song.formattedDuration})\``
    });
...
distube.on("addSong", (queue, song) => {
  try {
  queue.textChannel.send({ content: `> <a:VorRteX:1171926655078506547>  *Added* \`${song.name}\` - \`(${song.formattedDuration})\` **to the queue by ${song.user}**` }),
...
distube.on("addList", (queue, playlist) => {
  try {
    queue.textChannel.send({ content: `> <a:VorRteX:1171926655078506547>  *Added* \`${playlist.name}\` __**playlist**__ **\`(${playlist.songs.length} songs)\` ** to queue**` }),
